...
bool single_union_find(vector<int> &res1 , vector<int> &res2,vector<vector<int> >&A,int c){ // 判断两个簇是否能合并；能的话返回true；
...
int main()
{
    ...
    int father[n]; //并查集算法，记录每个节点的祖先节点；
    ...
    while(k > 1){
        ...
        for(auto i = 0 ; i < reg.size()-1 ; i++){ // 依次读取上一次划分结果的簇，然后寻找合并可能；
            for(auto j = i+1 ; j < reg.size() ; j++){
              //  show_class(A);
                if(single_union_find(K_c[i],reg[j],A,c)){ // 能够合并的话；
                      //   k--;
                    father[j] = father[i];
